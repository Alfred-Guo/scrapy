...
def springer(response):
    ...
    for i in range(1,4):
        for i2 in range(1,10):
            key=response.xpath('//div[@class="product-bibliographic"]/dl/dd/'\
                               'div/div/dl[%s]/dt[%s]/text()'%(i,i2)).extract_first()
            info=response.xpath('//div[@class="product-bibliographic"]/dl/dd/'\
                                'div/div/dl[%s]/dd[%s]/text()'%(i,i2)).extract()
            if not key:
                continue
            if key=='Editors' or key=='Authors':
                info=';'.join(response.xpath(\
                    '//div[@class="product-bibliographic"]/dl/dd/div/div/'\
                    'dl[%s]/dd[%s]/ul//span/text()'%(i,i2)).extract()
                )
            elif key=='Publisher':
                info=response.xpath(\
                    '//div[@class="product-bibliographic"]/dl/dd/div/div/'\
                    'dl[%s]/dd[%s]/span/text()'%(i,i2)).extract_first()
            elif key=='Topics':
                info=';'.join(response.xpath(\
                    '//div[@class="product-bibliographic"]/dl/dd/div/div/'\
                    'dl[%s]/dd[%s]/ul/li/a/text()'%(i,i2)).extract()
                )
            elif info:
                info=info[0]
            if key in keys.keys():
